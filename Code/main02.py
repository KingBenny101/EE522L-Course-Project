"""
    The reference implementation uses bytes and in this case the
    least significant bit of the first byte will be treated as index 0.
"""
...
class Grain128:
    def __init__(self, key, iv) -> None:
        ...
        self.lsfr = np.concatenate(
            (np.atleast_1d(hex_to_ndarray(iv, 96)), np.ones(32, dtype=np.uint8))
        )

    def _reset(self) -> None:
        self.nsfr = hex_to_ndarray(self.key, 128)
        self.lsfr = np.concatenate(
            (np.atleast_1d(hex_to_ndarray(self.iv, 96)), np.ones(32, dtype=np.uint8))
        )
